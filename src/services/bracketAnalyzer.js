...
class BracketAnalyzer {
  ...
  formatAnalysisResult(result, deck) {
    let message = `**Deck: ${deck.name}**\n`;
    message += `**Commander(s):** ${deck.commanders.join(', ')}\n`;
    message += `**Source:** ${deck.source}\n\n`;
    
    if (result.bracket !== null) {
      message += `✅ **Bracket ${result.bracket}: ${result.bracketName}**\n\n`;
      
      if (result.details) {
        message += `**Card Counts:**\n`;
        message += `• Tutors: ${result.details.counts.tutors}\n`;
        message += `• Two-Card Combos: ${result.details.counts.twoCardCombos}\n`;
        message += `• Game Changers: ${result.details.counts.gameChangers}\n`;
        message += `• Land Denial: ${result.details.counts.landDenial}\n`;
        
        // Show found cards if any
        for (const [category, cards] of Object.entries(result.details.foundCards)) {
          if (cards.length > 0) {
      ...
      if (result.bannedCards) {
        message += `\n**Banned Cards:**\n`;
        message += result.bannedCards.map(card => `• ${card}`).join('\n');
      }
      
      if (result.violations) {
        message += `\n**Violations:**\n`;
        message += result.violations.map(v => `• ${v}`).join('\n');
      }
...
module.exports = new BracketAnalyzer();
