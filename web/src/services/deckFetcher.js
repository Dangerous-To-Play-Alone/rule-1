...
class DeckFetcher {
  ...
  async fetchMoxfieldDeck(url) {
    try {
      ...
      // Moxfield API endpoint (using CORS proxy)
      const moxfieldUrl = `https://api2.moxfield.com/v3/decks/all/${deckId}`;
      const apiUrl = `https://corsproxy.io/?${encodeURIComponent(moxfieldUrl)}`;
...
export default new DeckFetcher();
